...
if page == "Data Upload & Processing":
    ...
    with st.sidebar:
        ...
        curr_isolate = st.session_state.super_subcat_pairs_params['isolate_super_subs']
        isolate_options = [curr_isolate, True if curr_isolate==False else False]
        isolate_lables = [isolate_options[0],isolate_options[1]]
        selected_isolate = st.radio("Isolate Partitioning Super-Subcategory Counterparts",isolate_options,index=0)
        if selected_isolate != st.session_state.super_subcat_pairs_params['isolate_super_subs']:
            st.session_state.super_subcat_pairs_params.update({'isolate_super_subs':selected_isolate })
            B)   st.session_state.kruskal_assumption_check_params
         
        
        # return pseudo{'full_pseudo':False}
        curr_ret_p = st.session_state.kruskal_assumption_check_params['return_pseudo']
        retp_options = [curr_ret_p, True if curr_ret_p==False else False]
        retp_lables = [retp_options[0],retp_options[1]]
        selected_retp = st.radio("Test Similarity Instead When Strict Mean Assumptions Arn't Met\nIn Kruskal-Wallis Test",retp_options,index=0)
        ...
        ps_options = [curr_ps, True if curr_ps==False else False]
        ps_lables = [ps_options[0],ps_options[1]]
        selected_ps = st.radio("Test Distribution Similarity Instead Mean\nIn Kruskal-Wallis Test",ps_options,index=0)
    ...
    # Page navigation
    page = st.radio("Navigate", ["Data Upload & Processing", "Model Fitting", "Visualization Groups", "Visualize Targets"], index=0)

    #=================================================================================================================================================================


elif page == "Model Fitting":
    if     is_ready_for_fit == False:
        ...
        if (not( not first_fit)) and is_fit==False:
            if "st.session_state.AD" not in st.session_state:
                st.session_state.st.session_state.AD = AnalyzeDataset(multivariate_params = st.session_state.cat_univar_params,
        ...
        elif (not (not refit)) and first_fit==False:
        ...
        if (not (not refit)) or (not (not first_fit)):
            st.session_state.st.session_state.AD.fit_full_dataset_analysis(st.session_state.data,  
...
if page in [ "Visualization Groups", "Visualize Targets"]:
    if     is_fit == False:
        ...
        with st.sidebar:
            ...
            if page == "Group Visualizations":
                ...
                univar_and_bivar_col_lists = [
                                            {'num_univar':st.session_state.AD.reject_null_normal,'num_univar_params':num_univar_params.copy().update({'super_title':'Numerical Non-Normal'})} ,
                                            {'num_univar':st.session_state.AD.fail_to_reject_null_normal,'num_univar_params':num_univar_params.copy().update({'super_title':'Numerical Normal'})}  , 
                                            {'cat_univar':st.session_state.AD.reject_null_good_of_fit,'cat_univar_params':cat_univar_params.copy().update({'super_title':'Categorical Non-Uniform'})} ,  
                                            {'cat_univar':st.session_state.AD.fail_to_reject_null_good_of_fit'cat_univar_params':cat_univar_params.copy().update({'super_title':'Categorical Uniform'})} , 
                                            {'numnum_bivar':st.session_state.AD.above_threshold_corr_numnum,'numnum_bivar_params':numnum_bivar_params.copy().update({'super_title':'Numeric-Numeric With Correlation'})}  , 
                                            {'numnum_bivar':st.session_state.AD.below_threshold_corr_numnum,'numnum_bivar_params':numnum_bivar_params.copy().update({'super_title':'Numeric-Numeric Without Correlation'})} , 
                                            {'numcat_bivar':st.session_state.AD.reject_null_numcat,'numcat_bivar_params':numcat_bivar_params.copy().update({'super_title':'Numeric-Categoric Reject Null'})}  , 
                                            {'numcat_bivar': st.session_state.AD.fail_to_reject_null_numcat,'numcat_bivar_params':numcat_bivar_params.copy().update({'super_title':'Numeric-Categoric Fail to Reject Null'})}  , 
                                            {'catcat_bivar':st.session_state.AD.reject_null_catcat,'catcat_bivar_params':catcat_bivar_params.copy().update({'super_title':'Categoric-Categoric Reject Null'})}  , 
                                            {'catcat_bivar':st.session_state.AD.fail_to_reject_null_catcat,'catcat_bivar_params':catcat_bivar_params.copy().update({'super_title':'Categoric-Categoric Fail to Reject Null'})}  , 
                                            {'super_subcat_pairs':st.session_state.AD.supercategory_subcategory_pairs,'super_subcat_pairs_params':super_subcat_pairs_params.copy().update({'super_title':'Categoric Partitioned by Another Categoric'})}   
                                            ]
                ...
                # call plot funciton
                st.session_state.st.session_state.AD.produce_all_plots(st.session_state.data,
                                            **default_params)
                ...
                if plot_selections:
                    if ( not variable_selection) or (not plot_type_selection):
                        ...
                        st.session_state.st.session_state.AD.visualize_by_targets(

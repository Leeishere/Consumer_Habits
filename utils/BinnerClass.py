...
class Bin(CompareColumns):
    ...
    def find_min_bins(self, data, y_col, x_columns, test_func, threshold, direction_of_relationship,method:str|None=None):
            ...
            while low<=high:
                ...
                if mid==2:
                    if (method in ('pearson','spearman')) and (direction_of_relationship=='lower'):
                        if stat<threshold:
                            min_relation_max_no_relation_stat[0]=1                                    
                            min_relation_max_no_relation_stat[1]=None
                            min_relation_max_no_relation_stat[2]=np.nan
                            break
        ...
        global_min_for_these_columns=max(mins_for_each_col)  # find the max of all the mins for this y_col. That min that is consistent through all varables
